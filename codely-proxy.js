...
/* ─── 请求体捕获（排查用）：完整请求体落盘 /tmp/codely-req/，保留最近 50 份 ─── */
const CAPTURE_DIR = '/tmp/codely-req';
function captureRequest(body, model) {
  try {
    fs.mkdirSync(CAPTURE_DIR, { recursive: true });
    const ts = new Date().toISOString().replace(/[:.]/g, '-');
    const file = path.join(CAPTURE_DIR, `${ts}_${model || 'unknown'}.json`);
    fs.writeFileSync(file, body);
    // 只保留最近 50 份
    const files = fs.readdirSync(CAPTURE_DIR).sort();
    while (files.length > 50) fs.unlinkSync(path.join(CAPTURE_DIR, files.shift()));
    log('capture', `请求体已捕获: ${file} (${body.length}B)`);
  } catch (e) { log('capture', `捕获失败: ${e.message}`); }
}

/* ─── 请求体改写：注入会话标识 ───
 * 另：Codely 网关会拒绝 system 消息含「ZCode」字样的请求（防竞品仿冒，实测 400
 * 「欢迎使用Codely/Turn execution failed」），转发前改写为中性名。 */
const BLOCKED_TEST = /zcode/i;          // 测试用（无 g，避免 lastIndex 状态坑）
const BLOCKED_NAME = /zcode/gi;         // 替换用
const NAME_REPLACEMENT = 'CodeAssistant';
function scrubText(s) { return s.replace(BLOCKED_NAME, NAME_REPLACEMENT); }

/** 深度擦洗对象里所有字符串值（工具参数 schema 等嵌套结构） */
function scrubDeepStrings(node) {
  let hits = 0;
  if (Array.isArray(node)) { for (const v of node) hits += scrubDeepStrings(v); return hits; }
  if (node && typeof node === 'object') {
    for (const k of Object.keys(node)) {
      const v = node[k];
      if (typeof v === 'string') { if (BLOCKED_TEST.test(v)) { node[k] = scrubText(v); hits++; } }
      else if (v && typeof v === 'object') hits += scrubDeepStrings(v);
    }
  }
  return hits;
}

/* 全请求身份擦洗（模仿官方 Codely CLI 请求，绕上游竞品名过滤）：
 * - messages：system / developer / assistant 全擦（user 实测不触发过滤，且可能含用户代码，不动）
 * - tools：function.description 与 parameters schema 全擦；function.name 不动
 *   （改了会导致模型回的 tool_call 名与客户端对不上，函数调用直接坏） */
function sanitizeRequest(j) {
  let hits = 0;
  if (Array.isArray(j.messages)) {
    for (const m of j.messages) {
      if (m.role === 'user') continue;
      if (typeof m.content === 'string') {
        if (BLOCKED_TEST.test(m.content)) { m.content = scrubText(m.content); hits++; }
      } else if (Array.isArray(m.content)) {
        for (const p of m.content) {
          if (p && typeof p.text === 'string' && BLOCKED_TEST.test(p.text)) { p.text = scrubText(p.text); hits++; }
        }
      }
      // 工具调用历史里的参数也可能带竞品名
      if (Array.isArray(m.tool_calls)) {
        for (const tc of m.tool_calls) {
          if (tc?.function?.arguments && BLOCKED_TEST.test(tc.function.arguments)) {
            tc.function.arguments = scrubText(tc.function.arguments); hits++;
          }
        }
      }
    }
  ...
  if (Array.isArray(j.tools)) {
    for (const t of j.tools) {
      const f = t?.function; if (!f) continue;
      if (typeof f.description === 'string' && BLOCKED_TEST.test(f.description)) { f.description = scrubText(f.description); hits++; }
      hits += scrubDeepStrings(f.parameters);
    }
  }
  return hits;
}
...
function transformBody(urlPath, body) {
  ...
  try {
    ...
    const hits = sanitizeRequest(j);
    if (hits) log('rewrite', `已擦洗 ${hits} 处竞品名（模仿官方请求） model=${j.model || '?'}`);
    ...
    const up = https.request({
      ...
      if (code >= 400) {
        ...
        ur.on('end', () => {
          ...
          // ① 401/403 → 刷新密钥后重试（密钥可能失效）
          if (code === 401 || code === 403) {
            const denied = /team_model_access_denied|not allowed to access model|model_access_denied/i.test(text);
            if (denied) {
              log('key', `上游 ${code}: 模型 ${model || '(未知)'} 被团队权限拒绝（透传，不重试）`);
              return resolve({ retry: false, model, passthrough: { status: code, headers: ur.headers, body: errBody } });
            }
            log('key', `上游返回 ${code}，刷新密钥后重试`);
            return resolve({ retry: true, status: code, errBody });
          }
          // ② 5xx → 服务端瞬时故障，自动重试（最多 2 次，不刷新密钥）
          if (code >= 500) {
            log('proxy', `上游 ${code} 服务端错误，自动重试 model=${model || '(未知)'}`);
            return resolve({ retry: true, refreshKey: false, status: code, errBody });
          }
          // ③ 其余所有 4xx（含 400）→ 客户端错误或上游业务逻辑拒绝，原样透传，不重试
          log('proxy', `上游 ${code}: ${text.slice(0, 120).replace(/\n/g, ' ')} （透传，不重试） model=${model || '(未知)'}`);
          return resolve({ retry: false, model, passthrough: { status: code, headers: ur.headers, body: errBody } });
        });
        return;
      }
...
async function handle(req, res, body) {
  ...
  // 排查用：捕获发往上游的完整请求体（探测请求除外）
  if (!isProbe && req.method === 'POST' && req.url.includes('/chat/completions')) {
    captureRequest(body, (() => { try { return JSON.parse(body.toString('utf8')).model; } catch { return null; } })());
  }
  let lastErr = null;
    ...
    try {
      ...
      if (r.retry) {
        ...
        if (r.refreshKey) {
          ...
          log('proxy', `上游 ${r.status} 自动重试 (${attempt + 1}/2)`);
        }
        continue;
      }
      if (r.passthrough) {
        // 权限拒绝/客户端错误：原样透传上游错误
        if (!isProbe) log('proxy', `${req.method} ${req.url} -> ${r.passthrough.status} (${Date.now() - started}ms, 透传${r.model ? `, model=${r.model}` : ''})`);
